...
class liveEMG():
    def __init__(self, cwd, filename_notlive):
        """Initialize a liveEMG session

        Args:
            cwd: directory where to find filename_notlive
            filename_notlive: filename of one .mat file where the recorded notlive data has been stored
        """
    ...
    def fit(self, classifier='NaiveBayes', X=[],y=[],split = 0):
        ...
        Args:
            classifier (str): Classifier string. Default = 'NaiveBayes'
            X (np.array): Matrix of size N_trials x N_features
            y (np.array): Vevtor of length N_trials, including the classifier for each trial
    ...
    def classify_live(self, data_live):
        ...
        Args:
            data_live: an object of class NST_EMG_LIVEa
    ...
    def RMS_features_extraction(self, data, trialList):
        """Calculates the RMS features within the period when the posture is performed
        """
    ...
    def split_dataset(self, N):
        """This function takes only the first N trials for training.

        Args:
            N: Number of trials for training

        Remarks:
            This function can be used for testing the effect of training data size on testing.
